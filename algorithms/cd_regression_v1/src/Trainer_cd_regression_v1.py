...
from models.losses import BinRotLoss, WeightedBCELoss
...
class Trainer_cd_regression_v1:
    def __init__(self, args, device, exp_id):
        self.args = args
        self.device = device
        self.exp_id = exp_id
        self.writer = self.set_writer(log_dir = "algorithms/" + self.args.algorithm + "/results/tensorboards/" + self.args.exp_name + "_" + exp_id + "/")
        self.checkpoint_name = "algorithms/" + self.args.algorithm + "/results/checkpoints/" + self.args.exp_name + "_" + exp_id
        self.plot_dir = "algorithms/" + self.args.algorithm + "/results/plots/" + self.args.exp_name + "_" + exp_id + "/"

        Dataset = get_dataset('custom')
        opt = update_dataset_info_and_set_heads(args, Dataset)
        print(opt)
        ...
        if opt.val_intervals < opt.epochs or opt.test:
            ...
            if opt.test:
                ...
                self.val_loader.dataset.run_eval(preds, self.checkpoint_name + '.pt')
                return
    ...
    def set_device(self, chunk_sizes, device):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch + 1, opt.epochs + 1):
            ...
            if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
                save_model(self.checkpoint_name + '.pt', 
